...
- LLM API key. Configure `LLM_PROVIDER` and optional comma-separated `LLM_FALLBACK_PROVIDERS` for an ordered chain. Production uses Azure OpenAI Responses (`gpt-5.6-sol`) → Gemini (`gemini-3.5-flash`) → Cloudflare Workers AI (`@cf/meta/llama-3.3-70b-instruct-fp8-fast`) → OpenRouter Ultra → OpenRouter Super. Azure uses the full `/openai/v1` base URL in `AZURE_OPENAI_BASE_URL`, appends `/responses`, authenticates with `AZURE_OPENAI_API_KEY` or `AZURE_OPENAI_API_KEY_FILE`, and has a bounded timeout. Each tier is attempted once; fallback occurs for invalid validated output, network/timeouts, or HTTP 408/409/429/5xx. Authentication and other non-retryable 4xx errors fail immediately. See `.env.example`.
### Installation Steps
